...
Compatible with GNU `tail` for line selection (`-n N`, `-n +N`) and the default last-ten-lines behavior; verified byte-identical in the `yupsh/bin-tail` Docker integration harness against Debian coreutils. Byte mode (`-c N`) and multiple file operands diverge as noted below. Follow mode (`-f`) is not implemented.
